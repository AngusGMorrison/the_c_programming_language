...
// Allocate n objects of nbytes
void *calloc(int nobjects, unsigned nbytes) {
    unsigned total_size = nobjects * nbytes;
    char *block_start = malloc(total_size);
    if (block_start == NULL) {
        return NULL;
    }

    char *block_current = block_start;
    while (block_current < block_start + total_size) {
        *block_current++ = 0;
    }
